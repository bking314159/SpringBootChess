	...
	xmlhttp.onreadystatechange = function() {
	  if (this.readyState == 4 && this.status == 200) {
		...
		for (var i=0; i<board.promotionPieces.length; i++) {
			...
			var image = document.createElement("img");
			image.src = "img/White" + board.promotionPieces[i] + "DarkBackground.png";
			data.appendChild(image);
...
function updateBoard(startId, endId) {
	...
	var url = "board.json?x1=" + startId.replace(".", "&y1=") + "&x2=" + endId.replace(".", "&y2=");
	...
	for (var i=0; i<board.squares.length; i++) {
		if (board.squares[i].x == x && board.squares[i].y == y) {
			if (board.squares[i].piece == null) {
				...
				return "img/" + board.squares[i].piece.owner + board.squares[i].piece.type + background;
			}
		}
	}
	return background;
